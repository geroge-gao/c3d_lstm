...
def load_video_list(path):
  ...
  videopath: the path of the files
  framcant: the frames of the video
...
def prepare_isogr_rgb_data(image_info):
  ...
  this part is to crop the images and Downsampling
  the images to a fixed sequence length
  '''
  ...
  scale=0，帧数小于要求输入帧数，前面的帧依次复制，不足的用最后一帧赋值
  scale=1，乘以相应的倍数，取对应的帧
  scale=0，
  scale=1，
  scale>1,帧数大于或等于帧数，并且不超过两倍，乘相应的倍数，取对应的帧，并添加正态分布偏置。
  '''
  if is_training:
    if scale == 0:
  ...
  for idx in range(0, output_frame_cnt):
    ...
    #data augment,which can make the computer better understand the images
